...
wire1.forEach(element => {
  const d = element.substr(0, 1);
  const num = parseInt(element.substr(1));

  for (var i = 0; i < num; i++) {
    if (d === "R") {
      wire1PointsMap[`${++x}_${y}`] = ++totalStepsSum;
    } else if (d === "L") {
      wire1PointsMap[`${--x}_${y}`] = ++totalStepsSum;
    } else if (d === "U") {
      wire1PointsMap[`${x}_${++y}`] = ++totalStepsSum;
...
y = 0;
totalStepsSum = 0;
let steps = 9999999999;
function updateSteps(x, y, index) {
  if (steps > x + y) {
    steps = x + y;
  }
}
wire2.forEach((element, index) => {
  const d = element.substr(0, 1);
  const num = parseInt(element.substr(1));
  ...
  for (var i = 0; i < num; i++) {
    totalStepsSum++;
    if (d === "R") {
      if (wire1PointsMap[`${++x}_${y}`]) {
        updateSteps(wire1PointsMap[`${x}_${y}`], totalStepsSum, index);
      }
    } else if (d === "L") {
      if (wire1PointsMap[`${--x}_${y}`]) {
        updateSteps(wire1PointsMap[`${x}_${y}`], totalStepsSum, index);
